...
def test_ws_open_when_no_token_env(monkeypatch):
    ...
    with TestClient(server.app) as client:
        with client.websocket_connect("/ws/snap") as ws:
...
def test_ws_rejects_wrong_token(monkeypatch):
    ...
    with TestClient(server.app) as client:
        with pytest.raises(WebSocketDisconnect) as ei:
            ...
            with client.websocket_connect("/ws/snap?token=WRONG") as ws:
                ws.receive_json()
...
def test_ws_accepts_right_token(monkeypatch):
    monkeypatch.setenv("WS_AUTH_TOKEN", "sekrit")
    with TestClient(server.app) as client:
        with client.websocket_connect("/ws/snap?token=sekrit") as ws:
            snap = ws.receive_json()
...
def test_ws_cap_rejects_overflow(monkeypatch):
    ...
    with TestClient(server.app) as client:
        with client.websocket_connect("/ws/snap") as ws1:
            ws1.receive_json()  # slot taken
            with pytest.raises(WebSocketDisconnect) as ei:
                with client.websocket_connect("/ws/snap") as ws2:
                    ws2.receive_json()
